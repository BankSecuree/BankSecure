...
conexao = mysql.connector.connect(user='user_bankSecure', password='Urubu_100', host='localhost', database='bankSecure', auth_plugin = "mysql_native_password")
...
def pegar_dados():
    ...
    # DIRETÓRIO PARA LINUX
     ...
     cpu_porcentagem_uso_label.config(text=f'Porcentagem de uso\n{cpu_porcent:.2f}%')
     cpu_velocidade_label.config(text=f'Velocidade\n{cpu_speed:.2f}GHz')
     cpu_velocidade_maxima_label.config(text=f'Velocidade Maxima\n{cpu_speed_max:.2f}GHz')
...
senha_entry = ttk.Entry(login_frame, width=25)
...
cpu_porcentagem_uso_label = ttk.Label(main_frame, text="Porcentagem de uso\n")
...
cpu_porcentagem_uso_label.grid(column=3, row=5, columnspan=2, padx=5, pady=5)
